...
function getPagingLinkExam($sql, $itemPerPage = 10, $strGet = '')
{
	$result        = dbQuery($sql);
	$pagingLink    = '';
	$totalResults  = dbNumRows($result);
	...
	if ($totalPages > 1) {
		...
		if ($pageNumber > 1) {
			$page = $pageNumber - 1;
			if ($page > 1) {
				$prev = " <a href=\"$self?class=$class&name=$name&page=$page&$strGet/\">[Prev]</a> ";
			} else {
				$prev = " <a href=\"$self?class=$class&name=$name&$strGet\">[Prev]</a> ";
			}	
				
			$first = " <a href=\"$self?class=$class&name=$name&$strGet\">[First]</a> ";
		} else {
			$prev  = ''; // we're on page one, don't show 'previous' link
			$first = ''; // nor 'first page' link
		}
		...
		if ($pageNumber < $totalPages) {
			$page = $pageNumber + 1;
			$next = " <a href=\"$self?class=$class&name=$name&page=$page&$strGet\">[Next]</a> ";
			$last = " <a href=\"$self?class=$class&name=$name&page=$totalPages&$strGet\">[Last]</a> ";
		} else {
			$next = ''; // we're on the last page, don't show 'next' link
			$last = ''; // nor 'last page' link
		}
		...
		for($page = $start; $page <= $end; $page++)	{
			if ($page == $pageNumber) {
				...
				if ($page == 1) {
					$pagingLink[] = " <a href=\"$self?class=$class&name=$name&$strGet\">$page</a> ";
				} else {	
					$pagingLink[] = " <a href=\"$self?class=$class&name=$name&page=$page&$strGet\">$page</a> ";
				}	
			}
