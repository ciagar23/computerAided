...
function addUser()
{
	...
	*/	
	...
	$sql = "SELECT user_idnumber
	        FROM tbl_student
			WHERE user_idnumber = '$IdNumber' and user_class ='$Class'";
	$result = dbQuery($sql);
	
	if (dbNumRows($result) == 1) {
		header('Location: index.php?view=add&class='.$Class.'&error=' . urlencode('ID Number already Registered. Log in or Choose another one'));	
	} else {			
		$sql   = "INSERT INTO tbl_student (user_idnumber ,user_class ,user_fname,user_lname, user_password, user_regdate)
		          VALUES ('$IdNumber','$Class','$FName','$LName', PASSWORD('$Password'), NOW())";
	
		dbQuery($sql);
		header('Location: index.php?view=list&class='.$Class.'&success=' . urlencode('You have Successfully been Registered'));	
	}
}
