...
function doLogin()
{
	...
	if ($IdNumber == '') {
		$errorMessage = 'You must enter your ID Number';
	} else if ($password == '') {
		...
		$sql = "SELECT user_id
		        FROM tbl_user 
				WHERE user_idnumber = '$IdNumber' AND user_password = PASSWORD('$password')";
		$result = dbQuery($sql);
	
		if (dbNumRows($result) == 1) {
			$row = dbFetchAssoc($result);
			$_SESSION['gso_user_id'] = $row['user_id'];
			...
			$errorMessage = 'Wrong ID Number or password';
		}		
...
{
	if (isset($_SESSION['gso_user_id'])) {
		unset($_SESSION['gso_user_id']);
		session_unregister('gso_user_id');
	}
...
function copyImage($srcFile, $destFile, $w, $h, $quality = 75)
{
    ...
    {
       $destFile  = substr_replace($destFile, 'pdf', -3);
